/**
 * 六爻纳甲插件:qigua(起卦)/paipan(排盘)/duangua(断卦辅助)/cha(查卦)。
 *
 * 独立模块:只含六爻(书3火珠林/书4增删卜易/书6卜筮正宗)业务,不依赖其他术别。
 * 数据来源:全部取自 知识库/data/*.json(经 ../lib/db 惰性加载),
 * 卦辞/爻辞/纳甲/六亲/六神/旬空/月破/旺相休囚等一律查数据,不硬编码。
 * 共享计算核心在 ../lib/hex,公历→干支换算复用 ../lib/ganzhi。
 */
...
async function duangua(args: {
  ...
  const bianName = hex.bianGuaName(gua, pan.dongs);
  out.push(
    "",
    `【相似卦例】(占验佐证,据 guaili.json·卦例库 ${db.loadGuaili().length} 则)`,
    ...guailiSimilar(args.占事, gua.卦名, bianName),
  );
...
/** cha 追加段:朱熹卦爻注(书8) + 序卦·杂卦(书1) */
function jingzhuanSegments(guaName: string, dongs: number[]): string[] {
  const out: string[] = [];
  const hit = zhuxiGuaNote(guaName);
  out.push(`【朱熹注】(据 books/08_周易本义.json·${hit?.卷 ?? "卷注"})`);
  if (hit) {
    const g = hit.卦;
    const gz = g.卦辞注?.[0];
    if (gz) out.push(`  ·卦辞注「${gz.经文}」→ ${clip(gz.注)}`);
    (g.爻注 ?? []).forEach((y: { 爻位: string; 爻辞: string; 注: string }, i: number) => {
      const dong = dongs.includes(i + 1);
      out.push(`  ·${y.爻位}${dong ? " ★动★" : ""}「${y.爻辞}」→ ${clip(y.注)}`);
    });
  } else {
    out.push(`  书8 无此卦注。`);
  }
  const sy = db.loadShiyi() as {
    序卦: { 卦序: number; 卦名: string; 内容: string }[];
    杂卦: { 卦名: string; 内容: string }[];
    文言?: { 乾: { 内容: string }[]; 坤: { 内容: string }[] };
  };
  const idx = db.guaIndex(guaName);
  const xq = sy.序卦?.find((x) => x.卦序 === idx);
  out.push(`【序卦·杂卦】(据 十翼.json)`);
  out.push(`  ·序卦第${xq?.卦序 ?? "?"}位: ${xq ? clip(xq.内容, 40) : "未寻得"}`);
  const zg = sy.杂卦?.find((x) => x.卦名 === guaName);
  out.push(`  ·杂卦: ${zg ? clip(zg.内容, 40) : "未寻得"} — 古人以此一言点出「${guaName}」性情`);
  if ((guaName === "乾" || guaName === "坤") && sy.文言?.[guaName]?.[0]) {
    out.push(`  ·文言传首段: ${clip(sy.文言[guaName][0].内容, 70)}`);
  }
  return out;
}

/** 占事→卦例库匹配关键词(据 guaili.json 各占事实际用语扩展 yongShenFor 分类) */
const GUILI_KW: Record<string, string[]> = {
  求财: ["求财", "财", "经营", "开铺", "贸易", "营生", "经商"],
  功名: ["功名", "升迁", "官", "仕", "科", "试", "谒贵"],
  婚姻: ["婚", "嫁", "娶", "妻", "姻"],
  疾病: ["病", "疾", "癌", "痘", "瘟", "伤", "症"],
  出行: ["出行", "行", "旅", "舟", "游", "往"],
...
/** duangua 追加段:相似卦例(占验佐证),按 占事分类+卦名/变卦相同 就近选取 1-3 则 */
function guailiSimilar(占事: string, guaName: string, bianName: string): string[] {
  const cat = Object.keys(GUILI_KW).find((k) => k !== "其他" && 占事.includes(k)) ?? "其他";
  const kws = GUILI_KW[cat] ?? [];
  const matches = db.loadGuaili()
    .map((e) => ({ e, score: 0 }))
    .map(({ e }) => ({
      e,
      score: (kws.some((k) => e.占事.includes(k)) ? 1 : 0) +
        (hex.shortGuaName(e.卦名) === guaName ? 2 : 0) +
        (hex.shortGuaName(e.变卦) === bianName ? 1 : 0),
    }))
    .filter((x) => x.score >= 1)
    .sort((a, b) => b.score - a.score || (a.e.占事 > b.e.占事 ? 1 : -1))
    .slice(0, 3);
  if (!matches.length) return [`  卦例库中暂无比类近例(占事${cat}类、卦${guaName}均无匹配)。`];
  const out: string[] = [];
  matches.forEach(({ e }, i) => {
    const line = e.来源?.[0]?.行 ?? "—";
    const book = e.书号 === 4 ? "增删卜易" : e.书号 === 6 ? "卜筮正宗" : `书${e.书号}`;
    const 白话 = e.白话?.replace(/\s+/g, "") || e.应期原理?.replace(/\s+/g, "") || "(无白话摘要)";
    out.push(
      `  ${["①", "②", "③"][i]} 占「${e.占事}」 卦:${e.卦名} 变:${e.变卦}  ${e.干支}`,
      `     白话: ${clip(白话, 55)}`,
      `     来源: ${book}(书${e.书号})·${e.卷}·行${line}`,
    );
  });
  out.push(`  白话解读: 古人占「${占事}」同类之事,见上例如何取用神、据旺衰空破而定吉凶应期,可资参照。`);
  return out;
}
...
async function cha(args: { 卦名: string; 动爻?: number[] }): Promise<string> {
  ...
  out.push("", ...jingzhuanSegments(gua.卦名, dongs), "");
  out.push("数据出处: 卦辞/变卦据 liushi_si_gua.json;爻辞/用九用六据 爻辞.json;易林据 yilin.json;八卦象意据 bagua.json;朱熹注据 books/08_周易本义.json;序卦杂卦文言据 十翼.json", 口径披露());
  return out.join("\n");
}
...
/** 六爻核心工具自声明(供聚合器合并) */
export const 元信息 = { 名: "六爻纳甲", 书号: [2, 3, 4, 6], 法式: ["纳甲筮法", "三枚铜钱六掷", "梅花时间起卦", "报数起卦", "字占起卦"] };
